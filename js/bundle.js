...
function setFocusedBTC() {
    var results = document.querySelectorAll('#btc');
    for (result of results) {
        result.classList.add('focused-indigo-max');
    }
}
function unsetFocusedBTC() {
    var results = document.querySelectorAll('#btc');
    for (result of results) {
        result.classList.remove('focused-indigo-max');
    }
}
function setFocusedAlt() {
    var results = document.querySelectorAll('#alt');
    for (result of results) {
        result.classList.add('focused-indigo-max');
    }
}
function unsetFocusedAlt() {
    var results = document.querySelectorAll('#alt');
    for (result of results) {
        result.classList.remove('focused-indigo-max');
    }
...
for (result of results) {
    result.addEventListener("focusin", setFocusedBTC);
    result.addEventListener("focusout", unsetFocusedBTC);
}
var r = document.querySelectorAll('input[type="text"]');
for (r of r) {
    r.addEventListener("focusin", setFocusedAlt);
    r.addEventListener("focusout", unsetFocusedAlt);
}

var uf = require('unfuck');
document.getElementById("outputtext").style.display = "none";
function compileIt(elementValue, inputText) {
    var y = elementValue;
    var x = inputText;
    compiler = uf.compiler()
    return(compiler.run(y, x))
}

function compileUse(elementValue) {
    var y = elementValue;
    compiler = uf.compiler()
    return(compiler.use(y))
}
document.getElementById("run").addEventListener("click", function () {
    var x = document.getElementById("outputtext");
    x.value = compileIt(document.getElementById("codetext").value, document.getElementById("stdin").value);
        if (x.style.display === "none") {
            x.style.display = "inline-block";
        }
})
document.getElementById("c2js").addEventListener("click", function () {
    var x = document.getElementById("outputtext");
    x.value = compileUse(document.getElementById("codetext").value);
    if (x.style.display === "none") {
        x.style.display = "inline-block";
    }
})
},{"unfuck":2}],2:[function(require,module,exports){
